...
void hmb_lock(unsigned long *flags)
{
	...
	atomic64_inc(&HMB_CTRL.t_waiting_lock_count_total[cpu_id]);

	t_cur = hmb_get_current_time_ns();
	while(!spin_trylock(&HMB_CTRL.lock_local))
	{
		is_waited = true;
		wait_for_completion(&HMB_CTRL.lock_completion);
	}
	...
	*flags = tmp_flags;
	...
	if(is_waited)
	{
		atomic64_inc(&HMB_CTRL.t_waiting_lock_count[cpu_id]);
		atomic64_add(hmb_get_current_time_ns() - t_cur, &HMB_CTRL.t_waiting_lock_accTime[cpu_id]);
	}
}
...
void hmb_unlock(unsigned long *flags)
{
	...
	complete(&HMB_CTRL.lock_completion);
...
void hmb_meta_update(void)
{
	...
	if(unlikely(HMB_CTRL.hmb_enabled == false || \
				meta == NULL ||
				meta->C__table_ST_seg_id < 0))
	{
		if(meta != NULL)
		{
			hmb_debug("Incorrect request. (C__table_ST_seg_id: %d)", \
	...
	init_completion(&HMB_CTRL.lock_completion);
}
...
bool hmb_RC_copy_from_HMB(struct bio *b)
{
	...
	for(i=0; i<sector_num; i++)
	{
		...
		if(i == sector_num-1)
		{
			uint64_t diff = sector_idx;

			if(i == 0)
			{
				diff = (diff << 9) % HMB_CTRL.cache_unit;

				remainder_hmb = sector_num << 9;
				written_hmb = diff;
			}

			else
			{
				remainder_hmb = ((sector_idx + sector_num) << 9) % HMB_CTRL.cache_unit;
