		...
		NvmeCmd *cmd, NvmeRequest *req)
{
	...
	if(HMB_CTRL.enable_hmb && req->is_write)
	{
		...
		nlp = hmb_spaceMgmt_nlb_to_nlp(req->nlb, req->slba);
		...
		else
		{
			...
			else
			{
				...
				for (i = 0; i < nlp; ++i)
				{
					...
					if(i == nlp-1)
					{
						uint64_t diff, diff_bits, j;

						if(i == 0)
						{
							diff = (req->slba << HMB_SPACEMGMT_CTRL.sector_size_bits) % HMB_SPACEMGMT_CTRL.cache_unit;
							diff_bits = diff >> HMB_SPACEMGMT_CTRL.sector_size_bits;

							hmb_remainder = req->nlb << HMB_SPACEMGMT_CTRL.sector_size_bits;
							hmb_offset = diff;

							for(j = 0; j<req->nlb; j++)
							{
								hmb_spaceMgmt_table_bm_set(true, entry_r->e_own, diff_bits + j);
							}
						}

						else
						{
							hmb_remainder = ((req->slba + req->nlb) << HMB_SPACEMGMT_CTRL.sector_size_bits) % HMB_SPACEMGMT_CTRL.cache_unit;
							if(hmb_remainder == 0)
							{
					...
					else if(i == 0)
					{
						...
						if((diff = (req->slba << HMB_SPACEMGMT_CTRL.sector_size_bits) % HMB_SPACEMGMT_CTRL.cache_unit))
						{
							diff_bits = diff >> HMB_SPACEMGMT_CTRL.sector_size_bits;
							hmb_offset = diff;
							hmb_remainder = HMB_SPACEMGMT_CTRL.cache_unit - diff;

							hmb_spaceMgmt_table_bm_set_fully(true, entry_r->e_own);
							for(j=0; j<diff_bits; j++)
							{
								hmb_spaceMgmt_table_bm_set(false, entry_r->e_own, j);
							}
						}
	...
	/*
	 * When FWB is only run in the SSD controller,
	 */
#if 0
	if(HMB_CTRL.enable_hmb)
	{
		if(req->is_write)
		{
			...
			diff = (int64_t)hmb_meta_get(false)->C__n_dirty + (int64_t)nlp - (int64_t)hmb_meta_get(false)->C__n_max_entries;

			if(diff > 0)
			{
				if(hmb_spaceMgmt_WB_flush_explicit(diff, &(n->ssd), &expire_time) == false)
				{
					hmb_spaceMgmt_unlock();
			...
			else
			{
				...
				for (i = 0; i < nlp; ++i)
				{
					...
					else
					{
						...
						/* hmb_spaceMgmt_RC_reorder() was already performed in the hmb_spaceMgmt_RC_caching() */
					}
					hmb_spaceMgmt_shared_set_enable(true, entry_r->e_own);
				}
				hmb_spaceMgmt_unlock();
		...
		else /* --> i.e. if(!req->is_write) */
		{
			...
			/* First of all, all data in NAND is copied to the host memory */
			...
			for(i=0; i<nlp; i++)
			{
				...
				if((entry_r = hmb_spaceMgmt_table_get_by_lpn(false, lpn_cur)) == NULL)
				{
					expire_time += SSD_READ(&(n->ssd), 1, lpn_cur);
					do_copy = false;
				}
				else
				{
					entry_w = hmb_spaceMgmt_table_get_by_idx(true, entry_r->e_own);
					src_base = hmb_spaceMgmt_mappedAddr_get_by_idx(false, entry_r->e_own);
				}
				...
				while(hmb_remainder > 0)
				{
					...
					if(hmb_remainder > iov_remainder)
					{
						if(do_copy)
						{
							memcpy(iov.iov[iov_idx].iov_base + iov_offset, \
									src_base + hmb_offset, \
									iov_remainder);
						}
#if 0
						...
						{
							memcpy(iov.iov[iov_idx].iov_base + iov_offset, \
									hs + req->data_offset + (HMB_SPACEMGMT_CTRL.cache_unit * i) + hmb_offset, \
									iov_remainder);
						}
#endif
				...
				if(do_copy)
				{
					...
					if(hmb_spaceMgmt_RC_reorder(entry_r->e_own) == false)
					{
						...
						hmb_debug("Failed to reorder RC entry.");
						goto HMB_ORIGINAL_FLOW;
					}
...
HMB_COMPLETE:
	...
	qemu_iovec_destroy(&iov); /* Bug fix: memory free! */
...
static void nvme_init_namespaces(NvmeCtrl *n)
{
    ...
    for (i = 0; i < n->num_namespaces; i++) {
		...
		hmb_debug("HMB_SPACEMGNT_CTRL.sector_size_bits : %d", HMB_SPACEMGMT_CTRL.sector_size_bits);
		hmb_debug("HMB_SPACEMGNT_CTRL.cache_unit_bits  : %d", HMB_SPACEMGMT_CTRL.cache_unit_bits);
